...
import { Leaf, TreePine, Clock, DollarSign, Car, Bus, TrendingUp, Award, Target } from 'lucide-react';
...
export function EcoImpactDashboard() {
  const [ecoStats] = useState<EcoStats>({
    todayTrips: 3,
    co2Saved: 2.4,
    moneySaved: 185,
    timeSaved: 25,
  ...
  useEffect(() => {
    ...
    const animateStats = () => {
      ...
      const interval = setInterval(() => {
        ...
        setAnimatedStats({
          co2: ecoStats.co2Saved * progress,
          money: ecoStats.moneySaved * progress,
          time: ecoStats.timeSaved * progress,
          trees: ecoStats.treesEquivalent * progress
        });
    ...
    <div className="min-h-screen bg-background p-4 pt-8 pb-24 overflow-y-auto">
      ...
      >
        ...
        <div className="text-center space-y-2">
          <h1 className="text-2xl font-bold bg-gradient-to-r from-neon-green to-accent bg-clip-text text-transparent" style={{ fontFamily: 'Montserrat, sans-serif' }}>
            Eco Impact
          </h1>
          ...
          <div className="space-y-4">
            ...
            <div className="text-center">
              <motion.p 
                ...
                transition={{ duration: 2, repeat: Infinity }}
              >
                +{animatedStats.time.toFixed(0)} minutes saved vs driving
              </motion.p>
            </div>
          </div>
        </Card>

        {/* Comparison: Bus vs Car */}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-3 p-3 bg-neon-green/10 rounded-lg border border-neon-green/20">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">₹{(ecoStats.moneySaved / ecoStats.todayTrips / 3).toFixed(0)}</span>
                  </div>
                  <div className="flex justify-between">
              ...
              <div className="space-y-3 p-3 bg-destructive/10 rounded-lg border border-destructive/20">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Cost/trip</span>
                    <span className="font-medium">₹{(ecoStats.moneySaved / ecoStats.todayTrips).toFixed(0)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">CO₂/trip</span>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {weeklyData.map((day, index) => (
                ...
                >
                  <div className="w-8 text-xs font-medium">{day.day}</div>
                  
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-muted-foreground">Trips: {day.trips}</span>
                      <span className="text-neon-green">{day.co2} kg CO₂</span>
                    </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="text-center space-y-2">
                ...
                >
                  {animatedStats.trees.toFixed(0)}
                </motion.p>
                <p className="text-xs text-muted-foreground">Trees Worth of CO₂ Saved</p>
              </div>
              
              <div className="text-center space-y-2">
                <DollarSign className="w-8 h-8 mx-auto text-neon-cyan" />
                <motion.p 
                  className="text-xl font-bold text-neon-cyan"
                  animate={{ scale: [1, 1.1, 1] }}
                  transition={{ duration: 3, repeat: Infinity, delay: 1 }}
                >
                  ₹{ecoStats.totalMoneySaved.toLocaleString()}
                </motion.p>
            ...
            <div className="text-center">
              <p className="text-sm text-muted-foreground">
                You've reduced your carbon footprint by <span className="text-neon-green font-semibold">{ecoStats.carbonFootprintReduction}%</span>
              </p>
            </div>
          </div>
        </Card>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-1 text-xs">
                ...
                <span className="text-neon-cyan">Rank #{ecoStats.rank.toLocaleString()}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {achievements.map((achievement, index) => {
                ...
                const progress = Math.min((achievement.progress / achievement.maxValue) * 100, 100);
        ...
        <Card className="p-4 bg-gradient-to-r from-neon-cyan/10 to-neon-blue/10 border-neon-cyan/20">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                <p className="text-lg font-bold text-neon-cyan">{ecoStats.totalUsers.toLocaleString()}</p>
